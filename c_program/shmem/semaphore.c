/****************************************************************************
  ...
  Provide a simple and easy to undestand interface to the System V
  semaphore system calls. There are 7 routines avalible to the user:

       id = sem_create(key, interval); # create with initial value or open
  ...
  The first member, [0], is the actual semaphore value, and the secound
  ...
  work if any prosess other than the last does an exit (intentional
  or unintentional).
  The third member, [2], of the semaphore set is used as a lock varible
...
static struct sembuf  op_endcreate[2] =
{
  {1, -1, SEM_UNDO},    /* decrement [1] (proc counter) with undo on exit */
		         ...
		         before explicity calling sem_close() */
...
/* initval is used if we create the semaphore */
{
	register int id, semval;

  /* The fourth argument to function semctl() is platform sepcific */
  ...
  /*
   * When the semsphore is created, we know that the value of all
   * 3 members is 0.
...
int sem_open(key_t key)
{
	...
	if ( (id = semget(key, 3, 0)) == -1)
    {
      printf ("does not exist or tables full");
     ...
     int   id;
{
  ...
  /*
   * The followering semop() first gets a lock on the semaphore,
  ...
  /*
   * Now that we have a lock, read the value og the process
...
/**************************************************
 * Wait intil a semaphore's value is greater than 0, then decrement
 * it by 1 and return.
...
void sem_op(int id, int value)
{
	...
	if ((ret = semop(id, &op_op[0], 1)) < 0)
	{
		#ifdef SEMAPHORE_DEBUG
    		printf("...sem_op error... %d\n", ret);
		#endif
	}
}
/*************************************************/
