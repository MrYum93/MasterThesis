        ...
        if not self.fix:
            if not coll:
                ...
                for i in range(len(objects)):
                    if self.x < objects[i].x + objects[i].w and self.x > objects[i].x - objects[i].w and self.y < \
                            objects[i].y + objects[i].w and self.y > objects[i].y - objects[i].w:
    ...
    if pt != None:
        ...
        try:
            ...
            for i in range(min_val, max_val, 8):
                newRope = RopeElement(map(i, min_val, max_val, prevX, newX), map(i, min_val, max_val, prevY, newY),
                                      None, None, False, objects, 0.05,
